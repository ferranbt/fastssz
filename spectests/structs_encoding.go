...
func (s *SigningRoot) UnmarshalSSZ(buf []byte) error {
	...
	if cap(s.ObjectRoot) == 0 {
		s.ObjectRoot = make([]byte, 0, 32)
	}
	s.ObjectRoot = append(s.ObjectRoot, buf[0:32]...)

	// Field (1) 'Domain'
	if cap(s.Domain) == 0 {
		s.Domain = make([]byte, 0, 8)
	}
...
func (b *BeaconState) UnmarshalSSZ(buf []byte) error {
	...
	if cap(b.GenesisValidatorsRoot) == 0 {
		b.GenesisValidatorsRoot = make([]byte, 0, 32)
	}
	...
	for ii := 0; ii < 8192; ii++ {
		if cap(b.BlockRoots[ii]) == 0 {
			b.BlockRoots[ii] = make([]byte, 0, 32)
		}
	...
	for ii := 0; ii < 8192; ii++ {
		if cap(b.StateRoots[ii]) == 0 {
			b.StateRoots[ii] = make([]byte, 0, 32)
		}
	...
	for ii := 0; ii < 65536; ii++ {
		if cap(b.RandaoMixes[ii]) == 0 {
			b.RandaoMixes[ii] = make([]byte, 0, 32)
		}
	...
	// Field (7) 'HistoricalRoots'
	{
		...
		for ii := 0; ii < num; ii++ {
			if cap(b.HistoricalRoots[ii]) == 0 {
				b.HistoricalRoots[ii] = make([]byte, 0, 32)
			}
...
func (b *BeaconStateAltair) UnmarshalSSZ(buf []byte) error {
	...
	// Field (15) 'PreviousEpochParticipation'
	{
		...
		if cap(b.PreviousEpochParticipation) == 0 {
			b.PreviousEpochParticipation = make([]byte, 0, 1099511627776)
		}
	...
	// Field (16) 'CurrentEpochParticipation'
	{
		...
		if cap(b.CurrentEpochParticipation) == 0 {
			b.CurrentEpochParticipation = make([]byte, 0, 1099511627776)
		}
...
func (s *SyncAggregate) UnmarshalSSZ(buf []byte) error {
	...
	if cap(s.SyncCommiteeBits) == 0 {
		s.SyncCommiteeBits = make([]byte, 0, 64)
	}
	...
	// Field (13) 'Transactions'
	{
		...
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			...
			if cap(e.Transactions[indx]) == 0 {
				e.Transactions[indx] = make([]byte, 0, 1073741824)
			}
...
func (e *ExecutionPayloadHeader) UnmarshalSSZ(buf []byte) error {
	...
	if cap(e.BaseFeePerGas) == 0 {
		e.BaseFeePerGas = make([]byte, 0, 32)
	}
	...
	if cap(e.TransactionsRoot) == 0 {
		e.TransactionsRoot = make([]byte, 0, 32)
	}
