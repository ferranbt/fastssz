...
// Hash: 6eecfa6999d66ce5b6db682d03b11ae0df77c0c65095829091a80df3909fbd90
// Version: 0.1.4
package spectests
...
func (h *HistoricalBatch) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'BlockRoots'
	if size := uint64(len(h.BlockRoots)); size != 8192 {
		err = ssz.ErrVectorLengthFn("HistoricalBatch.BlockRoots", size, 8192)
		return
	}
	for ii := uint64(0); ii < 8192; ii++ {
		dst = append(dst, h.BlockRoots[ii][:]...)
	}

	// Field (1) 'StateRoots'
	if size := uint64(len(h.StateRoots)); size != 8192 {
...
func (b *BeaconStateAltair) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (5) 'BlockRoots'
	if size := uint64(len(b.BlockRoots)); size != 8192 {
	...
	// Field (6) 'StateRoots'
	if size := uint64(len(b.StateRoots)); size != 8192 {
	...
	// Field (13) 'RandaoMixes'
	if size := uint64(len(b.RandaoMixes)); size != 65536 {
		err = ssz.ErrVectorLengthFn("BeaconStateAltair.RandaoMixes", size, 65536)
		return
	}
	for ii := uint64(0); ii < 65536; ii++ {
		if size := uint64(len(b.RandaoMixes[ii])); size != 32 {
	...
	// Field (14) 'Slashings'
	if size := uint64(len(b.Slashings)); size != 8192 {
	...
	// Field (9) 'Eth1DataVotes'
	if size := uint64(len(b.Eth1DataVotes)); size > 2048 {
...
func (s *SyncCommittee) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'PubKeys'
	if size := uint64(len(s.PubKeys)); size != 512 {
		err = ssz.ErrVectorLengthFn("SyncCommittee.PubKeys", size, 512)
		return
	}
	for ii := uint64(0); ii < 512; ii++ {
...
func (s *SyncAggregate) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := uint64(len(s.SyncCommiteeBits)); size != 64 {
...
func (e *ExecutionPayloadCapella) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := uint64(len(e.Withdrawals)); size > 16 {
